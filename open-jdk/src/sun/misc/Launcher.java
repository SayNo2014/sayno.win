...
public Launcher() {
    ...
    // 创建应用加载器（AppClassLoader），传入ExtClassLoader作为其父加载器
    try {
    ...
    // 绑定当前线程的类加载器为AppClassLoader
    Thread.currentThread().setContextClassLoader(loader);

    // 如果配置了安全管理器，则创建SecurityManager实例
...
{
	// 获取%JAVA_HOME%\jre\lib\ext目录下的所有文件
